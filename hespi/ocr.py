...
class TrOCR(OCR):
    def __init__(self, size:TrOCRSize=TrOCRSize.BASE):
        with warnings.catch_warnings():
            ...
            self.processor = TrOCRProcessor.from_pretrained(f'microsoft/trocr-{size}-handwritten')
            
            print("Getting VisionEncoderDecoderModel")
            self.model = VisionEncoderDecoderModel.from_pretrained(f'microsoft/trocr-{size}-handwritten')
